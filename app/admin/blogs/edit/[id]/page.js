...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/portfolio')
      ...
      .then((data) => {
        ...
        const post = blogs.find((b) => String(b.id) === String(editId));
        if (!post) {
          setMessage('Post not found');
          setLoading(false);
          return;
        }
        originalPost.current = post;
        setTitle(post.title || '');
        ...
        if (post.blocks && post.blocks.length > 0) {
          setBlocks(
            post.blocks.map((b, i) => ({
              id: b.id ?? Date.now() + i + Math.random(),
              type: b.type,
              content: b.content ?? (b.type === 'text' ? '' : null)
            }))
          );
        } else {
          setBlocks([{ id: Date.now(), type: 'text', content: post.content || '' }]);
        }
        setLoading(false);
      })
      .catch(() => {
  ...
  const handleUpdate = async () => {
    ...
    const post = originalPost.current;
    if (!post) return;
    setPublishing(true);
    setMessage('');
    try {
      ...
      const updated = existingBlogs.map((p) => (String(p.id) === String(editId) ? updatedPost : p));
        ...
        <div className="min-h-screen bg-white dark:bg-[#0d0d0d] flex">
          ...
          <div className="flex-1 lg:ml-64 pt-16 lg:pt-0 flex items-center justify-center">
            <div className="text-center">
              ...
              <Link href="/admin/blogs" className="text-black dark:text-white font-medium hover:underline">
                ← Back to blogs
              </Link>
            </div>
          </div>
      ...
      <div className="min-h-screen bg-white dark:bg-[#0d0d0d] flex">
        ...
        <div className="flex-1 lg:ml-64 pt-16 lg:pt-0">
          <div className="max-w-3xl mx-auto px-6 py-8">
            ...
            <h1 className="text-3xl font-bold text-black dark:text-white mb-6">Edit blog post</h1>
